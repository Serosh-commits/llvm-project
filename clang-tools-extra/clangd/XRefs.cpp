...
class FindControlFlow : public DynamicRecursiveASTVisitor {
...
public:
  ...
  bool VisitBreakStmt(BreakStmt *B) override {
    found(Break, B->getKwLoc());
    return true;
  }
  bool VisitContinueStmt(ContinueStmt *C) override {
    found(Continue, C->getKwLoc());
    return true;
  }
  bool VisitSwitchCase(SwitchCase *C) override {
    found(Case, C->getKeywordLoc());
    return true;
  }
  bool VisitCXXThrowExpr(CXXThrowExpr *T) override {
    found(Throw, T->getThrowLoc());
    return true;
  }
  bool VisitGotoStmt(GotoStmt *G) override {
