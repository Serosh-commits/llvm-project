...
namespace {
struct UnqualNameVisitor : public DynamicRecursiveASTVisitor {
public:
  ...
  bool TraverseTypeLoc(TypeLoc TL, bool TraverseQualifier = true) override {
    ...
    return DynamicRecursiveASTVisitor::TraverseTypeLoc(
        TL, TraverseQualifier);
  }

  // Replace the base method in order to call our own
  // TraverseTypeLoc().
  bool TraverseQualifiedTypeLoc(QualifiedTypeLoc TL, bool TraverseQualifier = true) override {
    return TraverseTypeLoc(TL.getUnqualifiedLoc(), TraverseQualifier);
  }

  bool VisitDeclRefExpr(DeclRefExpr *S) override {
